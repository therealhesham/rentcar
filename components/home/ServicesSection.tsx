  ...
  return (
    <section className="relative overflow-hidden bg-[#fdfbf6] px-4 py-20 sm:px-8 sm:py-28" dir="rtl">
      ...
      <div 
        ...
        style={{
          backgroundImage: "radial-gradient(circle, #003749 1.5px, transparent 1.5px)",
      ...
      <div className="relative mx-auto max-w-screen-xl">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-6 xl:gap-8">
          {services.map((service, index) => {
            // First 3 items take 2 columns each (3 items in row 1 for desktop)
            // Last 2 items take 3 columns each (2 items in row 2 centered for desktop)
            ...
            return (
              <article
                key={service.title}
                className={`group relative flex flex-col overflow-hidden rounded-[2rem] border border-[#ebe4d3]/60 bg-white/80 p-8 shadow-[0_8px_24px_-12px_rgba(15,61,71,0.08)] backdrop-blur-md transition-all duration-500 hover:-translate-y-1.5 hover:border-[#dbb878]/40 hover:bg-white hover:shadow-[0_20px_40px_-12px_rgba(219,184,120,0.15)] ${
                  mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                } ${colSpanClass}`}
                style={{ transitionDelay: mounted ? `${delay}ms` : "0ms" }}
              >
                {/* Decorative border highlight on top edge */}
