...
import { updateCarBookingsWhatsappSetting } from "./car-bookings-settings-actions";

export function CarBookingsSettingsModal({ initialWhatsapp }: { initialWhatsapp: string }) {
  const [isOpen, setIsOpen] = useState(false);
  const [whatsappList, setWhatsappList] = useState<string[]>(() => {
    return initialWhatsapp.split(",").map(w => w.trim()).filter(Boolean);
  });
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      ...
      const res = await updateCarBookingsWhatsappSetting(whatsappList.join(","));
      if (!res.ok) {
